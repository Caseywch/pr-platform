...
// For a change-request submission: Requester (they raised it), the specific
// Approver who approved this PR (role fallback if that stage somehow hasn't
// happened), and everyone currently holding Purchasing on the project (they
// decide it). Deliberately no Verifier — this only ever fires post-PO, well
// past the Verifier's stage, so including them would be noise.
async function changeRequestedEmails(supabase, pr) {
  const requester = await emailsForIds(supabase, [pr.requester_id]);
  const verifier = pr.verified_by
    ? await emailsForIds(supabase, [pr.verified_by])
    : await emailsForRole(supabase, pr.project_id, "verifier");
  const approver = pr.approved_by
    ...
    : await emailsForRole(supabase, pr.project_id, "approver");
  const purchasers = await emailsForRole(supabase, pr.project_id, "purchaser");
  return Array.from(new Set([...requester, ...verifier, ...approver, ...purchasers]));
}
...
export async function POST(request) {
  ...
  // A Requester just proposed a change on a post-PO PR. Recipients are the
  // full workflow group who already touched this PR — Requester (they
  // raised it), the specific Verifier and Approver who verified/approved
  // this PR, and everyone currently holding Purchasing on the project
  // (they're the one who actually decides it). The email needs the actual
  // before/after values, not just field names, so this fetches the
  // change-request row itself plus the project/supplier/uom names it
  // references (old and new may differ) and hands them to
  ...
  if (event === "pr_change_requested") {
    ...
    if (changeRequest) {
      ...
      const to = await changeRequestedEmails(supabase, pr);
      if (to.length > 0) {
