...
export default function UsersTab({ supabase, profiles, setProfiles, fail, router }) {
  ...
  const createUser = async () => {
    ...
    // Best-effort welcome email — quiet no-op until an email service is configured.
    fetch("/api/notify", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ event: "new_user", name: newName.trim(), email: newEmail.trim().toLowerCase(), pin: newPin }),
    }).catch(() => {});
