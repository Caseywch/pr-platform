...
// Builds a real field-by-field before/after diff for a change request,
// instead of the old "Items changed." summary that named the field but
// never showed what actually changed. lookups holds simple id->name maps
// so this works both client-side (with the full suppliers/projects/uoms
// arrays already in state) and inside the email template (which only has
// what's passed to it) — same shape either way, so the UI and the email
// never drift apart on what counts as "changed" or how it's described.
export function buildChangeRequestDiff(request, lookups = {}) {
  const projects = lookups.projects || {};
  const suppliers = lookups.suppliers || {};
  const uoms = lookups.uoms || {};

  const fields = [];
  if (request.old_project_id !== request.new_project_id) {
    fields.push({
      label: "Project",
      old: projects[request.old_project_id] || "\u2014",
      new: projects[request.new_project_id] || "\u2014",
    });
  }
  if (request.old_supplier_id !== request.new_supplier_id) {
    fields.push({
      label: "Supplier",
      old: suppliers[request.old_supplier_id] || "\u2014",
      new: suppliers[request.new_supplier_id] || "\u2014",
    });
  }
  if (request.old_request_date !== request.new_request_date) {
    fields.push({ label: "Request date", old: request.old_request_date, new: request.new_request_date });
  }
  if (request.old_required_date !== request.new_required_date) {
    fields.push({ label: "Required delivery date", old: request.old_required_date, new: request.new_required_date });
  }

  // Items are matched by position (same convention the rest of the app
  // uses for a PR's item list — there's no stable item id to match on
  // across old/new, since a change request can add or remove rows too).
  ...
  const describeItem = (it) =>
    it ? `${it.description || ""} \u00b7 qty ${it.qty ?? "\u2014"} ${uoms[it.uom_id] || ""}`.trim() : null;
  for (let i = 0; i < maxLen; i++) {
    const oldIt = oldItems[i] || null;
    const newIt = newItems[i] || null;
    const oldStr = JSON.stringify(oldIt);
    const newStr = JSON.stringify(newIt);
    if (oldStr === newStr) continue;
    itemChanges.push({
      index: i + 1,
      old: describeItem(oldIt),
      new: describeItem(newIt),
    });
  }

  return { fields, itemChanges, hasChanges: fields.length > 0 || itemChanges.length > 0 };
}
...
// Same shape as pendingCancelRequestsFor, but for change requests: only
// Purchasing ever decides these (no second Admin stage), so the role check
// is simpler. Matches the existing is_purchasing/is_admin check used above
// for consistency with how the rest of the app currently gates Purchasing
// actions, rather than introducing a different check here.
export function pendingChangeRequestsFor(changeRequests, prs, profile) {
  if (!profile) return [];
  if (!profile.is_purchasing && !profile.is_admin) return [];
  return (changeRequests || [])
    .filter((r) => r.status === "pending")
    .map((r) => {
      const pr = prs.find((p) => p.id === r.pr_id);
      return pr ? { ...pr, _changeRequest: r } : null;
    })
    .filter(Boolean);
}
