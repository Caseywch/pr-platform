...
export default async function BoardPage() {
  ...
  const [profileRes, prsRes, projectsRes, suppliersRes, uomsRes, allProjectRolesRes] = await Promise.all([
  ...
  const myRequesterProjectIds = new Set(
    allProjectRoles.filter((r) => r.role === "requester" && r.user_id === user.id).map((r) => r.project_id)
  );
  const projectsWithAnyRequester = new Set(
    allProjectRoles.filter((r) => r.role === "requester").map((r) => r.project_id)
  );
