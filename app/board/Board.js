...
import {
  ...
  autoCorrectAllCaps, activeChangeRequest, assignedNamesFor, localDate,
  buildChangeRequestDiff, pendingChangeRequestsFor,
} from "./prHelpers";
...
export default function Board({ profile, initialPrs, allProjects, eligibleProjects, suppliers: initialSuppliers, uoms, allProjectRoles, sla, initialCancelRequests = [], initialChangeRequests = [] }) {
  ...
  const startDuplicate = async (pr) => {
    ...
    const { data: sourceAttachments, error: attachError } = await supabase
      .from("pr_attachments")
      .select("*")
      .eq("pr_id", pr.id)
      .order("uploaded_at");
  ...
  // Matched by name rather than a hardcoded id so this keeps working regardless
  // of which environment's suppliers table it's pointed at.
  const isTbaSupplier = (pr) => (pr.suppliers?.name || "").trim().toUpperCase() === "TBA";

  // Fulfilled/Cancelled PRs drop out of everyday view 7 days after they
  // reached that state — nothing is deleted, "Show archived" brings them back.
  const ARCHIVE_AFTER_DAYS = 7;
  const isArchived = (pr) => {
    const finishedAt = pr.status === "fulfilled" ? pr.fulfilled_date : pr.status === "cancelled" ? pr.cancelled_at : null;
    if (!finishedAt) return false;
    const daysSince = (Date.now() - new Date(finishedAt).getTime()) / (1000 * 60 * 60 * 24);
    return daysSince >= ARCHIVE_AFTER_DAYS;
  };

  const visiblePrs = showArchived ? prs : prs.filter((p) => !isArchived(p));

  let filtered = statusFilter === "all" ? visiblePrs : visiblePrs.filter((p) => p.status === statusFilter);
  ...
  const onCreated = async (newPr, { excludedAttachmentIds } = {}) => {
    setPrs([newPr, ...prs]);
    setShowForm(false);
    if (duplicateSource) {
      try {
        await duplicateAttachments(supabase, duplicateSource.sourcePrId, newPr.id, excludedAttachmentIds);
      } catch (err) {
  ...
  const issuePo = async (pr) => {
    ...
    // A PO can't go out to "TBA" — Purchasing must pin down the real supplier
    // at this point, since this is the first stage where it's actually known.
    const chosenSupplierId = poSupplierDraft[pr.id];
    if (isTbaSupplier(pr) && !chosenSupplierId) return;
    ...
    if (isTbaSupplier(pr) && chosenSupplierId) patch.supplier_id = chosenSupplierId;
  ...
  const purchaserDecideChangeRequest = async (request, approve) => {
    ...
    if (approve) {
      ...
      const newItemRows = (request.new_items || []).map((i, idx) => ({
        pr_id: request.pr_id,
  ...
  const myActionCount =
    ...
    pendingChangeRequestsFor(changeRequests, prs, profile).length;
        ...
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-3">
            <button onClick={() => { setStatusFilter("all"); setDelayOnly(false); }} className="text-xs uppercase tracking-wide text-neutral-600">
              Show all ({visiblePrs.length})
            </button>
            <button onClick={() => setShowArchived(!showArchived)} className="text-xs uppercase tracking-wide text-neutral-600 underline">
              ...
              <div key={pr.id} className={card + " p-0"} style={expandedId === pr.id ? { borderColor: "#171717" } : undefined}>
                <div onClick={() => toggleExpand(pr)} className="w-full flex flex-wrap items-center justify-between gap-x-3 gap-y-1.5 px-4 py-3 text-left cursor-pointer">
                  ...
                  <span className="flex flex-wrap items-center gap-1.5">
                    ...
                    {isArchived(pr) && (
                      <span className="text-xs px-2 py-1 rounded-full" style={{ background: "#66666614", color: "#666666" }}>
                        Archived
                      </span>
                    )}
                  ...
                  <div className="pb-4 pt-1">
                            ...
                            {items.map((it) => (
                              <tr key={it.id} className="border-t border-neutral-100">
                                ...
                                <td className="py-1.5 pr-3 align-top">
                                  <div className="break-words" style={{ maxHeight: 72, overflowY: "auto", whiteSpace: "pre-wrap" }}>{it.description}</div>
                                </td>
                                ...
                                <td className="py-1.5 align-top">
                                  <div className="break-words" style={{ maxHeight: 72, overflowY: "auto", whiteSpace: "pre-wrap" }}>{it.remark || "—"}</div>
                                </td>
                              </tr>
                            ))}
                          </tbody>
                    ...
                    {pr.status === "pending_po" && canPurchase && (
                      <div className="bg-neutral-50 border border-neutral-200 rounded-md p-3">
                        ...
                        {isTbaSupplier(pr) && (
                          <div className="mb-2">
                            <select
                              ...
                              value={poSupplierDraft[pr.id] || ""}
                              onChange={(e) => setPoSupplierDraft({ ...poSupplierDraft, [pr.id]: e.target.value })}
                            >
                              <option value="">Select actual supplier *</option>
                              {suppliers.filter((s) => s.status !== "pending" && s.name.trim().toUpperCase() !== "TBA").map((s) => (
                                <option key={s.id} value={s.id}>{s.name}</option>
                              ))}
                            </select>
                            <div className="text-xs text-neutral-600 mt-1">Supplier was TBA at submission — pick who the PO is actually going to.</div>
                          </div>
                        )}
                        ...
                        <button
                          onClick={() => issuePo(pr)}
                          disabled={isTbaSupplier(pr) && !poSupplierDraft[pr.id]}
                        ...
                        <div className="flex flex-col gap-1">
                          {(eventsByPr[pr.id] || []).map((ev) => (
                            <div key={ev.id} className="text-xs text-neutral-600">
                              {localDate(new Date(ev.created_at))} — {ev.detail || ev.event_type}
                            </div>
                          ))}
                        </div>
                      </div>
                    ...
                    {(() => {
                      ...
                      const diff = buildChangeRequestDiff(changeRequest, {
                        projects: Object.fromEntries(allProjects.map((p) => [p.id, p.name])),
                        suppliers: Object.fromEntries(suppliers.map((s) => [s.id, s.name])),
                        uoms: Object.fromEntries(uoms.map((u) => [u.id, u.name])),
                      });
                      ...
                      return purchaserDecidingChangeId === pr.id ? (
                        <div className="bg-neutral-50 border border-neutral-200 rounded-md p-3 mt-2">
                          ...
                          {diff.hasChanges ? (
                            <div className="text-xs text-neutral-700 mb-2 flex flex-col gap-1.5">
                              {diff.fields.map((f) => (
                                <div key={f.label}>
                                  <span className="font-medium">{f.label}:</span>{" "}
                                  <span className="line-through text-neutral-400">{f.old}</span>{" "}
                                  <span>&rarr; {f.new}</span>
                                </div>
                              ))}
                              {diff.itemChanges.map((ic) => (
                                <div key={ic.index}>
                                  <span className="font-medium">Item {ic.index}:</span>
                                  {ic.old && (
                                    <div className="line-through text-neutral-400 pl-3">{ic.old}</div>
                                  )}
                                  {ic.new && <div className="pl-3">{ic.new || "(removed)"}</div>}
                                  {!ic.new && <div className="pl-3 text-neutral-400">(removed)</div>}
                                </div>
                              ))}
                            </div>
                          ) : (
                            <div className="text-xs text-neutral-600 mb-2">No fields appear changed.</div>
                          )}
...
function ChangeRequestForm({ supabase, pr, suppliers, uoms, initialItems, eligibleProjects, profile, onSubmitted, onError, onCancel, notify }) {
  ...
  const submit = async () => {
    ...
    if (notify) notify({ event: "pr_change_requested", prId: pr.id });
    onSubmitted(data);
  };
...
function NewPrForm({ supabase, eligibleProjects, suppliers, setSuppliers, uoms, allProjectRoles, profile, onCreated, onError, duplicateSource = null }) {
  ...
  const [excludedAttachmentIds, setExcludedAttachmentIds] = useState(new Set());
  const toggleExcludedAttachment = (id) => {
  ...
  const submit = async () => {
    ...
    setSubmitting(false);
    onCreated(prData, { excludedAttachmentIds });
  };
  ...
  return (
    <div className={card + " mb-5"}>
      ...
      {duplicateSource?.attachments?.length > 0 && (
        <div className="text-xs px-3 py-2 rounded-md mb-3 border border-neutral-300">
          <div className="text-neutral-600 mb-2">
            Attachments copied from the original requisition — remove any you don't want on this one:
          </div>
          {[
            { key: "quotation", label: "Quotations" },
            { key: "budget_comparison", label: "Budget Comparisons" },
            { key: "drawing", label: "Drawings" },
            { key: "photo", label: "Photos" },
          ].map(({ key, label }) => {
            const group = duplicateSource.attachments.filter((a) => a.category === key);
            ...
            return (
              <div key={key} className="mb-2 last:mb-0">
                ...
                <div className="flex flex-col gap-1">
                  {group.map((a) => {
                    const excluded = excludedAttachmentIds.has(a.id);
                    return (
                      <div key={a.id} className="flex items-center justify-between gap-2">
                        <span className={excluded ? "line-through text-neutral-400" : ""}>
                          {a.filename}
                          {a.category === "drawing" && a.drawing_number && (
                            <span className="text-neutral-500"> · Dwg {a.drawing_number} · Rev {a.revision_no}</span>
                          )}
                        </span>
                        <button
                          type="button"
                          onClick={() => toggleExcludedAttachment(a.id)}
                          className="text-xs underline text-neutral-600 shrink-0"
                        >
